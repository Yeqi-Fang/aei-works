...
def create_pairwise_comparison_matrices(data_dict, alpha=0.05):
    ...
    def extract_number(filename):
        import re
        # 查找 mismatches_ 后面的第一个数字
        match = re.search(r'mismatches_(\d+)', filename)
        return int(match.group(1)) if match else 0
    
    # Create mapping from original filename to clear display label
    def create_display_label(filename):
        import re
        # 提取所有数字：mismatches_X-Y-Z.csv
        match = re.search(r'mismatches_(\d+)-(\d+)-(\d+)', filename)
        if match:
            x, y, z = match.groups()
            return f"{x} {y} {z}"  # 或者使用 f"{x}{y}{z}" 如果您想要 "288" 格式
        return filename
