...
def cal_cost_overall_700yr_new_timing_model(               # ← same name!
    fmin, fmax, fdotmin, fdotmax,
    fsp, fdotsp,                                           # (unused)
    fband_cell, fdotband_cell,
    m0, m1, m2,
    r1, r2,
    Tcoh_day,
    nSFT, nSeg,
    ...
    fband_k   = np.arange(fmin + fband_cell/2,  fmax  - fband_cell/2 + fband_cell,   fband_cell)
    fdot_k    = np.arange(fdotmin + fdotband_cell/2, fdotmax - fdotband_cell/2 + fdotband_cell,
                          fdotband_cell)
    ...
    for f in fband_k:
        for fd in fdot_k:
            ...
            f_band_samp = ( f_sideband + fbandwidth
                            + 2e-4*f
                            + abs(fd)*T_span
                            + fddot*T_span**2
                            + 16/1800 )
